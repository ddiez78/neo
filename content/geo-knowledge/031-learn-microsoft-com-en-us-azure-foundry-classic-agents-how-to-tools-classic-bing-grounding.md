---
title: "How to use Grounding with Bing Search in Foundry Agent Service (classic) - Microsoft Foundry (classic) portal"
...
publishedTime: "Tue, 28 Apr 2026 22:12:38 GMT"
---
...
2.   Only paid and pay-as-you-go Azure subscriptions are eligible for Grounding with Bing Search. Sponsored subscriptions or subscriptions based on free credits are not eligible to deploy Grounding with Bing Search.
3.   By creating and using a Grounding with Bing Search resource through code-first experience, such as Azure CLI, or deploying through deployment template, you agree to be bound by and comply with the terms available at [https://www.microsoft.com/en-us/bing/apis/grounding-legal](https://www.microsoft.com/en-us/bing/apis/grounding-legal), which might be updated from time to time.
4.   When you use Grounding with Bing Search, your customer data is transferred outside of the Azure compliance boundary to the Grounding with Bing Search service. Grounding with Bing Search is not subject to the same data processing terms (including location of processing) and does not have the same compliance standards and certifications as the Foundry Agent Service, as described in the [Grounding with Bing Search Terms of Use](https://www.microsoft.com/en-us/bing/apis/grounding-legal). It is your responsibility to assess whether use of Grounding with Bing Search in your agent meets your needs and requirements.

The user query is the message that an end user sends to an agent, such as _"should I take an umbrella with me today? I'm in Seattle."_ Instructions are the system message a developer can provide to share context and provide instructions to the AI model on how to use various tools or behave.

When a user sends a query, the customer's AI model deployment first processes it (using the provided instructions) to later perform a Bing search query (which is [visible to developers](https://learn.microsoft.com/en-us/azure/foundry-classic/agents/how-to/tools-classic/bing-grounding#how-to-display-grounding-with-bing-search-results)). Grounding with Bing returns relevant search results to the customer's model deployment, which then generates the final output.
...
1.   Grounding with Bing Search works with [all Azure OpenAI models](https://learn.microsoft.com/en-us/azure/foundry-classic/agents/concepts/model-region-support) that Agent Service supports, except `gpt-4o-mini, 2024-07-18` and gpt-5 models.

1.   Create an Azure AI Agent by following the steps in the [quickstart](https://learn.microsoft.com/en-us/azure/foundry-classic/agents/quickstart).
...
[![Image 1: A screenshot of the Bing resource selection in the Azure portal.](https://learn.microsoft.com/en-us/azure/foundry-classic/agents/media/tools/bing/resource-selection.png)](https://learn.microsoft.com/en-us/azure/foundry-classic/agents/media/tools/bing/resource-selection.png#lightbox)

    2.   You can also create one through code-first experience. If so, you need to manually [register](https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/resource-providers-and-types#register-resource-provider) Bing Search as an Azure resource provider. You must have permission to perform the `/register/action` operation for the resource provider. The permission is included in the **Contributor** and **Owner** roles.

```
az provider register --namespace 'Microsoft.Bing'
```
3.   After you have created a Grounding with Bing Search resource, you can find it in [Azure portal](https://portal.azure.com/#home). Navigate to the resource group you've created the resource in, search for the Grounding with Bing Search resource you have created.

[![Image 2: A screenshot of the Bing resource in the Azure portal.](https://learn.microsoft.com/en-us/azure/foundry-classic/agents/media/tools/bing/resource-azure-portal.png)](https://learn.microsoft.com/en-us/azure/foundry-classic/agents/media/tools/bing/resource-azure-portal.png#lightbox)

When you add the Grounding with Bing Search tool to your agent, you can pass the following parameters. These parameters will impact the Grounding with Bing Search tool output, and the AI model might not fully use all of the outputs. See the [code examples](https://learn.microsoft.com/en-us/azure/foundry-classic/agents/how-to/tools-classic/bing-code-samples) for information on API version support and how to pass these parameters.
...
[![Image 3: A screenshot showing citations for Bing search results.](https://learn.microsoft.com/en-us/azure/foundry/agents/media/tools/bing/website-citations.png)](https://learn.microsoft.com/en-us/azure/foundry/agents/media/tools/bing/website-citations.png#lightbox)

See [code samples](https://learn.microsoft.com/en-us/azure/foundry-classic/agents/how-to/tools-classic/bing-code-samples) for using the Grounding with Bing tool programmatically.
