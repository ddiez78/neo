---
title: "How to use Custom Bing Search with Foundry Agent Service (classic) - Microsoft Foundry (classic) portal"
description: "Learn how to ground Azure AI Agents using Custom Bing Search results. (classic)"
url: "https://learn.microsoft.com/en-us/azure/foundry-classic/agents/how-to/tools-classic/bing-custom-search"
...
Once the resource is deployed, go to **Configurations** in the Microsoft Foundry portal, and create a configuration instance to include content only from websites that your users care about. Instructions about how to create your custom configuration can be found in the [setup section](https://learn.microsoft.com/en-us/azure/foundry-classic/agents/how-to/tools-classic/bing-custom-search#setup).
...
For an end-to-end example that prints citations from a thread message, see the [code samples](https://learn.microsoft.com/en-us/azure/foundry-classic/agents/how-to/tools-classic/bing-custom-search-samples).
...
2.   Include URLs to define your search slices in at least one of the following fields:
...
Grounding with Bing Custom Search only returns results for domains and webpages that are public and indexed by Bing.
    ...
    *   Webpage (for example, `https://www.microsoft.com/en-us/p/surface-earbuds/8r9cpq146064`)
    ...
    2.   Blocked domains to exclude from the search space.

3.   Determine if you want to include subpages. A subpage slice specifies a domain path. Bing searches all content found at and below the path. You can specify a maximum of two subfolders in the path. For example, `www.microsoft.com/windows/`.

4.   Adjust ranking if needed.

5.   Navigate to the **Agents** screen for your agent in the [Foundry portal](https://ai.azure.com/?cid=learnDocs), scroll down the Setup pane on the right to **knowledge**. Then select **Add**.

![Image 1: A screenshot of the agents screen in the Foundry portal.](https://learn.microsoft.com/en-us/azure/foundry-classic/agents/media/tools/knowledge-tools.png)

6.   Select the **Grounding with Bing Custom Search** tool.

7.   Select to create a new connection, or use an existing connection

    1.   For a new connection, select your Grounding with Bing Custom Search resource.

8.   Once you have connected to a resource, select the configuration name.

9.   Save the tool and start chatting with your agent.
...
2.   You can only block certain domains and perform a search against the rest of the Web (a competitor’s site, for example).

When you add the Grounding with Bing Custom Search tool to your agent, you can pass the following parameters. These parameters will impact the Grounding with Bing Custom Search tool output, and the AI model might not fully use all of the outputs. See the [code examples](https://learn.microsoft.com/en-us/azure/foundry-classic/agents/how-to/tools-classic/bing-custom-search-samples) for information on API version support and how to pass these parameters.
...
| `market` | The market where the results come from. Typically, `mkt` is the country/region where the user is making the request from. However, it could be a different country/region if the user isn't located in a country/region where Bing delivers results. The market must be in the form: `<language>-<country/region>`. For example, `en-US`. The string is case insensitive. For a list of possible market values, see [Market codes](https://learn.microsoft.com/en-us/bing/search-apis/bing-web-search/reference/market-codes). If known, you're encouraged to always specify the market. Specifying the market helps Bing route the request and return an appropriate and optimal response. If you specify a market that is not listed in Market codes, Bing uses a best fit market code based on an internal mapping that is subject to change. | String | No |
